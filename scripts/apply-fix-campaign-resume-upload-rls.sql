-- One-shot fix for shared-link resume upload failures.
-- Applies:
-- 1) Canonical campaign resume register function (uuid signature)
-- 2) Function grants for anon/authenticated
-- 3) storage.objects upload policy reset for campaign-reg path
-- 4) optional read policy for campaign-reg objects
  ...
  WITH CHECK (
    ...
    AND EXISTS (
      ...
      WHERE campaign.id::text = (storage.foldername(name))[2]
        ...
        AND (
          COALESCE(campaign.allowlisted_fields, '[]'::jsonb) ? 'resume'
          OR COALESCE(campaign.allowlisted_fields, '[]'::jsonb) ? 'platform_handles'
        )
    )
    AND EXISTS (
      SELECT 1
