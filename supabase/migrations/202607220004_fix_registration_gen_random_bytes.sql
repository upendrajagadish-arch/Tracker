-- Fix registration HTTP 404 caused by missing gen_random_bytes under search_path=public.
-- Prefer gen_random_uuid() (available on hosted Supabase with search_path=public).
...
BEGIN
  ...
  -- Avoid gen_random_bytes (pgcrypto) which is invisible when search_path is only public.
