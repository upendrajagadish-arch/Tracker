...
import { usePlacementTheme } from '@/hooks/usePlacementTheme'

type Props = {
  className?: string
}

export function ThemeToggleBulb({ className }: Props) {
  const { resolvedTheme, setTheme } = usePlacementTheme()
  ...
  return (
    <button
      ...
      className={cn(
        'inline-flex size-10 items-center justify-center rounded-xl border border-soft bg-elevated text-foreground transition-all duration-300 hover:border-primary/40 hover:bg-primary/10 active:scale-95',
        className,
      )}
